//
//  WRActivityIndicatorAnimationSystem.swift
//  WRSwiftUtilDemo
//
...
class WRActivityIndicatorAnimationSystem: WRActivityIndicatorAnimationDelegate {
    
    func setupAnimation(in layer: CALayer, size: CGSize, color: UIColor) {
        let lineSpacing: CGFloat = 2
        let lineSize = CGSize(width: min(4, ((size.width - 4 * lineSpacing) / 5)), height: (size.height - 2 * lineSpacing) / 3)
        let x = (layer.bounds.size.width - size.width) / 2
        let y = (layer.bounds.size.height - size.height) / 2
        let duration: CFTimeInterval = 1.2
        let beginTime = CACurrentMediaTime()
        let beginTimes: [CFTimeInterval] = [0.12, 0.24, 0.36, 0.48, 0.6, 0.72, 0.84, 0.96, 0.108, 0.12]
        #if swift(>=4.2)
    ...
    func lineAt(angle: CGFloat, size: CGSize, origin: CGPoint, containerSize: CGSize, color: UIColor) -> CALayer {
        ...
        let line = WRActivityIndicatorShape.line.layerWith(size: size, color: color)
